...
SAVE_PATH="/tmp/verl_checkpoints/sft_checkpoints/${save_path}"
...
torchrun --standalone --nnodes=$nnodes --nproc_per_node=$nproc_per_node --node_rank="${RANK}" --master_addr="${ADDR}" --master_port=$PORT \
    ...
    data.train_files=./verl_data/$dataset/train.parquet \
    data.val_files=./verl_data/$dataset/train.parquet \
