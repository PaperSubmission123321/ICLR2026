...
QUES2 = "What is the patient's PneumonA 76-year-old male resident of a nursing home is noted to have increased fatigue and change of appetite. He has a history of renal disease, is undergoing treatment for neoplastic disease, butia Severity Index (PSI)?"
RULES2 = [
	"Age: Enter age in years (age score will be equal to age in years)",
	"Sex: Female = -10 points, Male = 0 points",
	"Nursing home resident: No = 0 points, Yes = +10 points",
...
def get_data(rule: str) -> str:
	"""Accepts a rule and extracts the datapoint from the patient
...
def convert_units(rule: str, datapoint: str) -> str:
	"""Accepts a rule and a datapoint, and converts the datapoint
	to the correct unit to be evaluated in the rule, if necessary.
        """
	...

@mock.dictmock(check_rule_mockdict)
def check_rule(rule: str, relevant_data: str) -> int:
	"""Accepts a rule and the data necessary to determine how the
	rule applies, and returns an integer value corresponding to
...
def medcalc_rules(input_str):
	...
	"""
	###IF toplevel
	question, rules = analyze_input(input_str)
	acc = 0
	for rule in rules:
		relevant_data = get_data(rule)
